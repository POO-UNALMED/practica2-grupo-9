...
import exceptions.ExceptionsType1;
...
public class MenuTienda {
	...
	/*
	 * Este metodo mostrara por pantalla todos los productos almacenados en el
	 * carrito de compras del cliente Entrada: - productos es una lista que contiene
	 * todos los objetos de las subclases de productos almacenadas en carrito -
	 * cantidades es una lista que contiene las cantidades de cada producto que el
	 * cliente selecciono
	 */
	public static void mostrarListaProductos(List<Producto> productos, List<Integer> cantidades) {
		...
		if (!productos.isEmpty()) {
			...
			for (int i = 0; i < productos.size(); i++) {
				index = i + 1;
				System.out.println(
						"| " + index + "  |  " + productos.get(i).getCodigo() + "  |  " + productos.get(i).getNombre()
	...
	/*
	 * Encargado de agregar el producto y la cantidad seleccionada por el cliente al
	 * carrito de compras Entradas: -producto es el objeto de tipo producto
	 * seleccionado por el cliente -cantidad es la cantidad del producto
	 * seleccionado -productos es la lista que simula al carrito de compras y en la
	 * cual se almacena la instancia de producto -cantidades es lista que almacena
	 * las cantidades de los productos seleccionados por el cliente Salidas
	 * almacenar dichos datos en esas listas
	 */
	public static void agregarProducto(Producto producto, int cantidad, List<Producto> productos,
			List<Integer> cantidades) throws InventarioIncorrecto {
			...
			if (productos.indexOf(producto) == -1) {
				...
				if (cantidad <= producto.getInventario()) {
					...
					throw new InventarioIncorrecto(cantidad, producto.getInventario());
				}
			} else {
		...
		/*
		 * int index; int aumento; if (productos.indexOf(producto) == -1) {
		 * 
		 * if (cantidad <= producto.getInventario()) { productos.add(producto);
		 * cantidades.add(cantidad); System.out.println(Mensajes.exitoAgregar); } else {
		 * System.out.println(Mensajes.productoNoAgregado); } } else {
		 * 
		 * index = productos.indexOf(producto); aumento = cantidades.get(index) +
		 * cantidad;
		 * 
		 * if (cantidad <= producto.getInventario() && aumento <=
		 * producto.getInventario()) { cantidades.set(index, aumento);
		 * System.out.println(Mensajes.exitoAgregar); } else {
	...
	/*
	 * Encargado de validar si el producto seleccionado por el usuario existe en la
	 * base de datos y almacenando este al carrito si es correcto Entradas:
	 * -idProducto entero que representa el id del producto seleccionado por el
	 * cliente -cantida entero que representa la cantidad de dicho producto que
	 * quiere el cliente -productos es la lista que simula al carrito de compras y
	 * en la cual se almacena la instancia de producto -cantidades es lista que
	 * almacena las cantidades de los productos seleccionados por el cliente Salida
	 * Llama al metodo agregarProducto o en su defecto envia un mensaje de error al
	 * cliente
	 */
	...
	/*
	 * Encargado de eliminar parte del producto o en su totalidad del carrito de
	 * compras Entradas: -productos es la lista que simula al carrito de compras y
	 * en la cual se almacena la instancia de producto -cantidades es lista que
	 * almacena las cantidades de los productos seleccionados por el cliente -id
	 * representa el id del producto dentro del carrito de compras -cantidad
	 * representa la cantidad a eliminar de dicho producto en el carrito de compras
	...
	/*
	 * con este metodo hacemos la accion de pagar despues de que el cliente ya haya
	 * seleccionado los productos que va a llevar Entradas -productos es la lista
	 * que simula al carrito de compras y en la cual se almacena la instancia de
	 * producto -cantidades es lista que almacena las cantidades de los productos
	 * seleccionados por el cliente -cliente representa al cliente al cual va
	 * asociado la factura Salida Construye con estos datos una nueva factura
	 */
